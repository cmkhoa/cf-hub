...
const SuccessStory = () => {
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      } catch(err){ if(!cancelled){ console.error('Load success stories failed', err); } }
  ...
  // Split into 3 columns for display
  const columns = [[],[],[]];
    ...
    <section className="success-story-section">
      <div className="container">
        <div className="success-story-header">
          <Title level={2} className="success-story-title">Success Stories</Title>
              ...
              <Col xs={24} md={8} key={i}>
                <div className="category-column">
                  <div className="category-header">
                    <Title level={4} className="category-title">{i===0?'Latest Success Stories':''}</Title>
                      ...
                      <Card
                        ...
                        cover={
                          <div className="post-image-container small-image" onClick={()=> window.location.href = `/blog/${story.slug}` } style={{ cursor:'pointer' }}>
                            ...
                            />
                            <div className="post-tag">SUCCESS STORY</div>
                          </div>
                        }
